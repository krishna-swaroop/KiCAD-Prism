...
| `ws_release_approvals` | Project/config candidate/build approval, role/domains/decision, approver/note, nullable exception kind/reason, technical scope fingerprints, policy binding digest, diagnostic manifest digest, carried-from approval, re-auth context, optional evaluation, and creation time. Rows are immutable. |
...
Migration 9 hardening and the pre-development correction policy are documented
in [`R3a.md`](R3a.md) and [`R3c.md`](R3c.md). The migration
number/order remains 1 through 9 for this pre-development branch; R23 must fold
the ordered M9 body into M8 before the feature branch merges to dev, after
which migration append-only discipline begins.
