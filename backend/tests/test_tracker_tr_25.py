...
from app.api import tracker_sync as tracker_sync_api  # noqa: E402
...
class RoutingContractTests(unittest.TestCase):
    ...
    def test_promote_and_share_routes_registered(self) -> None:
        ...
        self.assertIn("/api/projects/{project_id}/comments/{comment_id}/promote", paths)
        self.assertIn("/api/projects/{project_id}/comments/{comment_id}/replies/{reply_id}/share", paths)
...
class PromotionPostgresTests(unittest.TestCase):
    def setUp(self) -> None:
        ...
        patches = [
            ...
            patch.object(tracker_sync_api, "comments_store", self.store),
            patch.object(tracker_sync_api, "get_project_for_role_or_404", return_value=self.project),
        ]
        for item in patches:
            item.start()
    ...
    def test_manual_promote_is_idempotent(self) -> None:
        ...
        designer = session("designer", user_id="u_designer")
        first = run(tracker_sync_api.promote_comment_to_tracker("prj_a", created["id"], designer))
        second = run(tracker_sync_api.promote_comment_to_tracker("prj_a", created["id"], designer))
    ...
    def test_designer_share_enqueues_add_comment(self) -> None:  # F8.share_to_github
        ...
        reply_id = posted["reply"]["id"]
        designer = session("designer", user_id="u_designer")
        shared = run(tracker_sync_api.share_reply_to_tracker("prj_a", comment_id, reply_id, designer))
    ...
    def test_viewer_promote_is_denied(self) -> None:
        viewer = session("viewer", user_id="u_viewer")
        created = self._create_pinned(viewer, commentClass="task", severity="info")
        response = run(tracker_sync_api.promote_comment_to_tracker("prj_a", created["id"], viewer))
