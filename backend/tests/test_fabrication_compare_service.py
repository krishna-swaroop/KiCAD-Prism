...
def test_parses_flashes_and_traces_into_board_millimetres():
...
def test_regenerating_the_same_board_reports_no_fabrication_change():
    body = (
        "X100000000Y100000000D02*\n"
        "X105000000Y100000000D01*\n"
    )
...
def test_renumbered_apertures_are_not_a_fabrication_change():
...
def test_a_resized_pad_is_one_changed_region_at_the_pad():
    base = parse_gerber(gerber(
        "X100000000Y100000000D03*\n",
        apertures="%ADD10C,0.250000*%\n",
    ))
    ...
    assert region["x"] == pytest.approx(99.7)
    assert region["y"] == pytest.approx(99.7)
    assert region["width"] == pytest.approx(0.6)


def test_a_pad_moved_across_the_board_is_a_removal_and_an_addition():
    ...
    assert regions[0].x == pytest.approx(99.875)
    assert regions[1].x == pytest.approx(139.875)


def test_a_trace_whose_start_moved_is_detected():
...
def test_nearby_changes_merge_into_one_reviewable_region():
...
def test_regions_are_numbered_top_down_then_left_to_right():
    base = parse_gerber(gerber(""))
    head = parse_gerber(gerber(
...
def test_polygon_regions_are_compared_by_outline():
...
def test_a_diffuse_change_is_many_local_markers_not_one_covering_the_board():
...
def test_rounded_rectangle_pads_are_measured_from_their_outline():
    ...
    assert aperture.half_extent[0] == pytest.approx(0.582, abs=0.01)
    assert layer.warnings == []


def test_an_unevaluatable_macro_is_flagged_rather_than_guessed():
    layer = parse_gerber(gerber(
        "X100000000Y100000000D03*\n",
        apertures=(
            "%AMWeird*\n"
...
def test_dark_and_clear_polarity_are_different_operations():
    base = parse_gerber(gerber("X100000000Y100000000D03*\n"))
...
def test_layer_names_survive_a_board_rename(tmp_path):
    """The board stem is part of every filename but not of the layer identity."""

    base = export_dir(tmp_path, "base", {"F_Cu": (".gtl", "Copper,L1,Top", PAD)}, stem="old-name")
    head = export_dir(tmp_path, "head", {"F_Cu": (".gtl", "Copper,L1,Top", PAD)}, stem="new-name")
...
def test_user_layers_are_not_collapsed_by_their_shared_file_function(tmp_path):
    ...
    base = export_dir(tmp_path, "base", layers)
    changed = dict(layers)
    changed["User_3"] = (".gbr", "Other,User", PAD_MOVED)
    head = export_dir(tmp_path, "head", changed)
...
def test_a_layer_only_one_revision_plots_is_reported_as_the_change(tmp_path):
    base = export_dir(tmp_path, "base", {
        "F_Cu": (".gtl", "Copper,L1,Top", PAD),
        "F_Paste": (".gtp", "SolderPaste,Top", PAD),
    })
    head = export_dir(tmp_path, "head", {"F_Cu": (".gtl", "Copper,L1,Top", PAD)})
...
def test_changed_layers_sort_ahead_of_unchanged_ones(tmp_path):
    base = export_dir(tmp_path, "base", {
        "B_Cu": (".gbl", "Copper,L2,Bot", PAD),
        "F_Cu": (".gtl", "Copper,L1,Top", PAD),
    })
    head = export_dir(tmp_path, "head", {
        "B_Cu": (".gbl", "Copper,L2,Bot", PAD),
...
def test_regions_are_reported_in_kicad_board_coordinates(tmp_path):
    ...
    base = export_dir(tmp_path, "base", {"F_Cu": (".gtl", "Copper,L1,Top", "")})
    head = export_dir(tmp_path, "head", {
    ...
    assert region["x"] == pytest.approx(99.875)
    assert region["y"] == pytest.approx(49.875)


def test_a_directory_without_a_job_file_still_yields_named_layers(tmp_path):
...
def test_drill_hits_carry_their_tool_diameter_and_plating():
...
def test_regenerating_the_same_drill_program_is_not_a_change():
    body = "T1\nX10.0Y-20.0\n"
...
def test_a_hole_that_stops_being_plated_is_a_fabrication_change():
...
def test_a_resized_drill_is_one_region_at_the_hole():
    ...
    assert regions[0].width == pytest.approx(0.5)


def test_canned_slots_are_compared_along_their_whole_length():
...
def test_routed_slots_record_the_path_the_tool_cuts():
...
def test_imperial_drill_programs_are_converted_to_millimetres():
    layer = parse_excellon(
        excellon(PLATED_VIA, "T1\nX1.0Y-2.0\n", units="INCH"),
    )

    assert layer.ops[0].points[0] == (pytest.approx(25.4), pytest.approx(-50.8))


def test_zero_suppressed_coordinates_are_scaled_by_the_format():
    layer = parse_excellon(excellon(PLATED_VIA, "T1\nX010000Y-020000\n"))

    assert layer.ops[0].points == ((10.0, -20.0),)


def test_the_drill_program_is_compared_beside_the_plotted_layers(tmp_path):
    ...
    base = package(tmp_path, "base", "T1\nX10.0Y-20.0\n")
    head = package(tmp_path, "head", "T1\nX10.0Y-20.0\nX14.0Y-20.0\n")
    ...
    assert drill["regions"][0]["y"] == pytest.approx(19.85)


def test_separate_plated_and_non_plated_programs_stay_separate(tmp_path):
    for name in ("base", "head"):
        directory = export_dir(tmp_path, name, {"F_Cu": (".gtl", "Copper,L1,Top", PAD)})
        (directory / "board-PTH.drl").write_text(excellon(PLATED_VIA, "T1\nX10.0Y-20.0\n"))
        (directory / "board-NPTH.drl").write_text(excellon(NON_PLATED, "T1\nX30.0Y-20.0\n"))

    result = compare_directories(tmp_path / "base", tmp_path / "head")
...
def test_a_flash_is_drawn_at_its_board_position():
...
def test_clear_polarity_paints_the_background_rather_than_the_layer():
    svg = render_layer_svg(
...
def test_a_trace_is_stroked_at_the_aperture_width():
    svg = render_layer_svg(
        parse_gerber(gerber(
...
def test_arcs_are_flattened_onto_their_true_radius():
...
def test_a_rounded_rectangle_pad_renders_its_macro_outline():
    svg = render_layer_svg(
        parse_gerber(gerber(
            "X010000000Y-005000000D03*\n",
            apertures=(
...
def test_both_revisions_render_into_one_shared_board_view(tmp_path):
    ...
    result = compare_directories(base, head, tmp_path / "render")
...
def test_inner_copper_layers_are_compared(tmp_path):
    ...
    def package(name, inner_body):
        directory = export_dir(tmp_path, name, {
            "F_Cu": (".gtl", "Copper,L1,Top", PAD),
        })
...
def test_a_drill_program_keeps_its_name_without_a_job_file(tmp_path):
