...
class RootMutationRouteTests(unittest.TestCase):
    ...
    def test_legacy_rows_are_admin_only_and_listings_carry_permissions(self) -> None:  # F1.legacy_admin_only
        ...
        # Publication needs a pinned anchor: an issue must deep-link to one
        # revision, and a legacy row never gains one by inference.
        self.assertEqual(row["permissions"], {"canReply": True, "canEdit": False, "canDelete": False, "canResolve": True, "canPublish": False, "canRetry": False})
        plugin = AuthenticatedUser(email="p@x", name="Plugin", role="designer", auth_type="kicad_provider")
